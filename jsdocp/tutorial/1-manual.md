...
Every SQL file used by `sqler` should be organized in a directory defined by each connection within a `conf.db.connections[].dir`. When the [Manager](Manager.html) is initialized (i.e. [Manager.init](Manager.html#init)) the directory is scanned for SQL files and generates a [Prepared Statement](https://en.wikipedia.org/wiki/Prepared_statement)/`async Function` for each file that is found. Each generated async function 
