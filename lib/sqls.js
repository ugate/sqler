 ...
 */
 class SQLS {
  ...
  async prepared(name, fpth, ext, parent, propertyName) {
    ...
    * @param {typedefs.SQLERExecOptions} [opts] The options
    * @param {String[]} [frags] The fragments
    * @param {(SQLERExecErrorOptions | Boolean)} [errorOpts] The error options
    */
    async function execSqlPublic(opts, frags, errorOpts) {
      ...
      // Any options that are added to the passed options must be explicitly set in xopts
      /** @type {typedefs.SQLERExecOptions} */
      const xopts = {
        name: opts && opts.name,
        type,
        stream: opts && Number.isInteger(opts.stream) && opts.stream >= 0 ? opts.stream : -1,
        binds,
