...
class Dialect {
   ...
   * @param {import('..').SQLERPrivateOptions} priv The private configuration options
   * @param {import('..').SQLERConnectionOptions} connConf The individual __connection__ configuration for the given dialect that was passed into the originating {@link Manager}
   * @param {import('..').SQLERTrack} [track] A tracking mechanism that can be used to share configuration between dialect implementations
   ...
   * @param {import('..').SQLERInitOptions} opts The initialization options
   ...
   * @param {import('..').SQLERTransactionOptions} opts The transaction options passed in via the public API
   * @returns {import('..').SQLERTransaction} The transaction that was started
   */
   ...
   * @param {import('..').SQLERExecMeta} meta The metadata used to generate the execution
   * @param {(import('..').SQLERExecErrorOptions | Boolean)} [errorOpts] The error options to use
   * @returns {import('..').SQLERExecResults} The execution results
   */
  ...
  /**
   * @returns {import('..').SQLERState} The state of the {@link Dialect}
   */
  get state() {
    return {
      pending: 0,
