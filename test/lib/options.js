...
const test = { mgr: null, cache: null, mgrLogit: typeof console.info === 'function' };
class Tester {
 ...
 static async valDialectsNull() {
  let conf = await UtilSql.initConf();
 ...
 static async valConnectionsDialectImportExternal() {
  const conf = await UtilSql.initConf();
  conf.db.dialects.external = 'node:assert';
 ...
 static async valMainPathEmpty() {
  ...
  conf.db.connections[0].driverOptions.numOfPreparedFuncs = 0;
  return UtilSql.initManager(test, conf, { skipPrepFuncs: true });
 }
 ...
 static async valInitErrorReturn() {
  const conf = await UtilSql.initConf();
  conf.db.connections[0].driverOptions = conf.db.connections[0].driverOptions || {};
  conf.db.connections[0].driverOptions.throwInitError = true;
