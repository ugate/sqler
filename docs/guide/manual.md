...
```
Each `conf.db.dialect` property should contain all of the [Dialect](/api/lib/dialect) vendor/driver implmentations used by the manager and can be set to either an extending Dialect class or a _path_ to an extended Dialect module. Many Dialects have already been implemented in separate modules that [listed in the README.md](/). The prior example calls `mgr.db.fin.ap.list.companies()` that uses the `conf.db.connections[].name = "fin"` as the property namespace under `db` on the manager instance.
...
- __A [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete) operation can be passed into the [generated SQL function](../api/typedefs#typedefs-sqlerpreparedfunction}) using the [`type` option](/api/typedefs#typedefs-sqlerexecoptions)__

Defining the _type_ of CRUD operation helps assist implementing [Dialect](/api/lib/dialect) to determine any supplemental processing that may need to take place (like transactional state).
...
<br/><br/>__[bind variables](/api/typedefs#typedefs-sqlerexecoptions) passed into the driver used by the implementing [Dialect](/api/lib/dialect):__
```json
{
  "someParam": "one",
  "someParam1": "two",
...
- [`rollback`](/api/typedefs#typedefs-sqlertransactionrollback-function) - Rolls back any pending changes from one or more previously invoked SQL script (pass `true` to release the connection back into the pool indicating that the transaction is complete)
...
const exec1 = await mgr.db.fin.create.ap.company(coOpts);
 ...
 Lets say there are multiple SQL scripts that need to be included in a single transaction. To do so, the `autoCommit` flag can be set to _true_ on the last transaction being executed. Also, to ensure every SQL script that is executed be performed within the same transaction scope, a `transactionId` should be set to the same value for every SQL execution that needs to be included within the same transaction. Calling `const tx = await manager.db.myConnectionName.beginTransaction()` will generate/return a [transaction](/api/typedefs#typedefs-sqlertransaction-object) that contains a unique `id` that can be passed into each [prepared function](/api/typedefs#typedefs-sqlerpreparedfunction-⇒-sqlerexecresults) [options](/api/typedefs#typedefs-sqlerexecoptions-object).
...
In `sqler`, prepared statements are handled internally via a chosen [Dialect](/api/lib/dialect) vendor implementation. Only the [`prepareStatement = true` flag](/api/typedefs#typedefs-sqlerexecoptions-object) needs to be set to indicate the underlying SQL script should be executed within a __dedicated request and/or connection__ from the pool. Once all of the SQL invokations are complete a call to `unprepare` from the [execution result](/api/typedefs#typedefs-sqlerexecresults-object) will ensure the statement/connection is closed.
...
Prepared statements can also be contained within a [transaction](#tx). When doing so, calls to `commit` or `rollback` on the [transaction](/api/typedefs#typedefs-sqlertransaction-object) will _implicitly_ call `unprepare` for each [execution result](/api/typedefs#typedefs-sqlerexecresults-object) that used the same [transaction](/api/typedefs#typedefs-sqlertransaction-object) on the [execution options](/api/typedefs#typedefs-sqlerexecoptions-object) that is passed into the [prepared function](/api/typedefs#typedefs-sqlerpreparedfunction-⇒-sqlerexecresults).
...
- __`typedefs.EVENT_STREAM_COMMIT`__ - Emitted when a read or write stream has committed a transaction. Typically, this will occur when [`autoCommit = true`](/api/typedefs#typedefs-sqlerexecoptions-object) and [`transactionId`](/api/typedefs#typedefs-sqlerexecoptions-object) has been set to a valid transaction ID.
- __`typedefs.EVENT_STREAM_ROLLBACK`__ - Emitted when a read or write stream has rolledback a transaction. Typically, this will occur when an `error` has been emitted on the stream and [`transactionId`](/api/typedefs#typedefs-sqlerexecoptions-object) has been set to a valid transaction ID.
- __`typedefs.EVENT_STREAM_RELEASE`__ - Emitted when a read or write stream has released or closed it's connection.

Example reads:
```js
const typedefs = require('sqle../ap../api/typedefs');
...
#### 🗄️ Caching SQL {#cache}
By default all SQL files are read once during [Manager.init](/api/manager#manager-init-returnerrors-⇒-typedefs-sqleroperationresults), but there are other options for controlling the frequency of the SQL file reads by passing a [cache container (see example)](/api/typedefs#typedefs-sqlercache-object) into the [Manager constructor](/api/manager#new-manager-conf-cache-logging) or by calling [Manager.setCache](/api/manager#manager-setcache-cache-istransfer-connnames-⇒-typedefs-sqleroperationresults).

Since `sqler` expects SQL files to be defined prior to [initialization](/api/manager#manager-init-returnerrors-⇒-typedefs-sqleroperationresults), there are several techniques that can be used to produce and maintain evolving SQL statements:
 1. Add/generate SQL files before [Manager.init](/api/manager#manager-init-returnerrors-⇒-typedefs-sqleroperationresults)
 1. Add/generate SQL files, then add the connection for them later using [Manager.addConnection](/api/manager#manager-addconnection-conn-priv-cache-logging-returnerrors-⇒-typedefs-sqleroperationresults)
