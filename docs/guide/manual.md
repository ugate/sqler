...
Prepared statements can also be contained within a [transaction](#tx). When doing so, calls to `commit` or `rollback` on the [transaction](/api/typedefs#.SQLERTransaction) will _implicitly_ call `unprepare` for each [execution result](/api/typedefs#.SQLERExecResults) that used the same [transaction](/api/typedefs#.SQLERTransaction) on the [execution options](../api/typedefs#typedefs-sqlerexecoptions) that is passed into the [prepared function](../api/typedefs#typedefs-sqlerpreparedfunction).
...
- __`typedefs.EVENT_STREAM_COMMIT`__ - Emitted when a read or write stream has committed a transaction. Typically, this will occur when [`autoCommit = true`](../api/typedefs#typedefs-sqlerexecoptions) and [`transactionId`](../api/typedefs#typedefs-sqlerexecoptions) has been set to a valid transaction ID.
- __`typedefs.EVENT_STREAM_ROLLBACK`__ - Emitted when a read or write stream has rolledback a transaction. Typically, this will occur when an `error` has been emitted on the stream and [`transactionId`](../api/typedefs#typedefs-sqlerexecoptions) has been set to a valid transaction ID.
...
Since `sqler` expects SQL files to be defined prior to [initialization](../api/lib/dbs), there are several techniques that can be used to produce and maintain evolving SQL statements:
 1. Add/generate SQL files before [Manager.init](../api/lib/dbs)
